...
from endpoints_models.store_item.model import ResponseStoreItemData, ResponseStoreItemsData
...
def test_get_store_item(app, store_item):
    """Get store item"""
    res = app.store_item.get_store_item(name_item=store_item[UserStore.ITEM].name,
...
def test_get_store_items(app, store_item):
    """Get store items"""
    res = app.store_item.get_store_items(headers=store_item[UserStore.HEADERS],
                                         type_response=ResponseStoreItemsData)
    assert res.status_code == 200, "Check code"
